...
void checkReset() {
  Firebase.getInt(firebaseData, "/sensors/PasswordStatus");
  int resetFlag = firebaseData.intData();

  if (resetFlag == 0) {
    // Perform hardware reset if resetFlag is 0
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Resetting Hardware...");
    delay(2000);  // Delay for reset message
    ESP.restart();  // Perform hardware reset (restart the ESP8266)
  } else if(resetFlag == 1) {
    // No reset, continue with normal operation
    PASSWORD();
